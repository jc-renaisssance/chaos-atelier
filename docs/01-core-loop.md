...
2. **Shop** → reshape material deck (buy / sell / sparse remove).
...
Goal: player **adjusts the deck** through the chapter cycle knowing the boss, like Slay the Spire act bosses.
...
4. Gear gets **stats + powers** (positive and **negative** synergies) from the resolver.
5. **Adventure simulation** — short report (tags / powers / outlook).
6. **Result** → gold, materials, reputation; scars Later.

## Phase-1 ship spine

Ship **one loop UI**: client brief → craft → adventure report → shop, with **chapter boss known at chapter start** even if full act map is stubbed.

Defer encyclopedia depth, special clients, and parallel meta until that holds.
